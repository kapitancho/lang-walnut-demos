...
replaceString = ^[String, String, String] => String :: #0->replace[match: #1, replacement: #2];
replaceRegExp = ^[String, RegExp, String] => String :: #0->replace[match: #1, replacement: #2];
...
test = ^[a: String<3..10>, b: String<..5>, c: String['a', 'hello'], d: String<1..10>, e: String<2..2>, f: MyString] => Map :: [
    ...
    concat: concat[#a, #c],
    concatMyString: concat[#f->value, #c],
    concatList: concatList[#a, [#c, #e]],
    split: split[#a, 'h'],
    chunk: chunk[#a, 3],
    padLeft: padLeft[#a, 20, '+-'],
    padRight: padRight[#a, 12, '+-'],
    replaceString: replaceString[#c, 'l', '1'],
    replaceRegExp: replaceRegExp[#c, RegExp('/l+/'), '*r*'],
    asString: asString(#a),
